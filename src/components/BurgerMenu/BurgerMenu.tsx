...
function BurgerMenu({ children }: PropsType) {
  ...
  function renderButton(Icon: ValidIconType, callback: () => void) {
    return (
      <Ripples color='rgba(255, 255, 255, .3)' className='menu-mobile-trigger-ripple menu-mobile-opener'>
        <button type="button" 
    ...
    <div className='menu-mobile'>
      ...
      {isMenuOpen && (
        <div className='menu-mobile-opened-container'>
          ...
          <div className="menu-mobile-opened-overlay" onClick={() => toggleMenu(false)} />
          <div className='menu-mobile-opened-content'>
            {renderButton(MdClose, () => toggleMenu(false))}  

            <div className="menu-mobile-opened-content-links">
              {children}
            </div>
          </div>
        </div>
